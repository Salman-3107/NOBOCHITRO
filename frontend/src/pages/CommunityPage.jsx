...
import { addPostComment, createPost, deletePost, deletePostComment, getPost, getUserProfile, likePost, listPosts, searchMovies, unlikePost } from '../api/movies';
...
function PostCard({ post, user, onChanged, onSelectMovie, onSelectProfile }) {
  ...
  async function toggleComments() {
    if (!expanded) { const detail = await getPost(post.POSTID); setComments(detail.comments || []); }
    setExpanded((current) => !current);
  }
  async function toggleLike() { setIsWorking(true); try { if (isLiked) { await unlikePost(post.POSTID); setLikes((v) => v - 1); } else { await likePost(post.POSTID); setLikes((v) => v + 1); } setIsLiked((v) => !v); } finally { setIsWorking(false); } }
  async function submitComment(event) { event.preventDefault(); if (!commentText.trim()) return; await addPostComment(post.POSTID, commentText.trim()); setCommentText(''); const detail = await getPost(post.POSTID); setComments(detail.comments || []); if (!expanded) setExpanded(true); onChanged(); }
  async function removeComment(commentId) { await deletePostComment(commentId); setComments((current) => current.filter((comment) => comment.COMMENTID !== commentId)); onChanged(); }
